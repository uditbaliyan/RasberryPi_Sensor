        ...
        function updateTextualReadings(temperature, pulse) {
            document.getElementById('heartRateReading').innerText = `Heart Rate: ${100}`;
            document.getElementById('temperatureReading').innerText = `Temperature: ${100} °C`;
        }
        ...
        if (heartRateChart.data.datasets[0].data.length >= maxReadings) {
            heartRateChart.data.labels.shift();
            heartRateChart.data.datasets[0].data.shift();
        }
        heartRateChart.data.labels.push(`Reading ${pulse}`);
